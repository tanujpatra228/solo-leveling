...
| Exercise help: how-to steps, common mistakes, and a muscle-map visual | **Done**, landed 2026-09-13 | A "?" next to every exercise on the live Gate screen (`gate.tsx`'s `ActiveBlockItem`, via the existing `HelpButton`) opens `ExerciseHelpModal` — a `SystemOverlay` showing the exercise's existing one-line `cue`, a new setup/steps/common-mistakes guide, and `MuscleMap`, a flat front-and-back body silhouette lit up by primary (bright) and secondary (dim) muscles. Guide copy lives in `content/exerciseGuides.ts`, keyed by exercise id rather than merged into the `Exercise` schema, covering all 41 *prescribed* exercises — fallback-library exercises are intentionally not covered yet (`ExerciseHelpModal` shows an honest "not written yet" rather than a placeholder for those); `exerciseGuides.test.ts` asserts every prescribed exercise has a guide and every guide resolves to a real id. The body map (`components/MuscleMap.tsx` + `muscleMapRegions.ts`) is hand-drawn inline SVG rectangles, not a raster asset or one image per exercise — 17 of the 20 `Muscle` values map to a silhouette region; the other three (`rotator_cuff`, `grip`, `cardio`) have no honest surface region on a flat body outline, so they render as a labelled badge instead of a fake highlight. A coverage test in `MuscleMap.test.tsx` asserts every `Muscle` enum value is covered by one or the other. All static content — no Worker, no D1, no sync — so none of the request/CPU/row budgets apply; only bundle size does, and the whole feature added ~8.6 KB gzipped (208.70 KB → 217.28 KB), still under half the Slow-4G budget. Also fixed, in passing: `routes.dom.test.tsx`'s "gate already cleared today" test read the real system clock for "today" rather than pinning one, so it failed on any Sunday (SEED_ROUTINES has no Sunday routine) — now pinned to a fixed Monday via `vi.setSystemTime`, the same convention `state.test.ts` already uses |
...
| Bundle | 217.28 KB JS + 6.56 KB CSS + 2.20 KB `workbox-window` ≈ 226.0 KB gzipped initial route, under half the ~480 KB Slow-4G budget. Only the Hunter License card and the QR scanner still ship as their own lazy chunks — Tower, Shadow Army and Shop were folded back into the main bundle 2026-09-10 (each under 1 KB, not worth Suspense's timing cost) |
