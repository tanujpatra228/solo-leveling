...
| Summon-list pop-in on Shadow Army / Demon Castle / System Shop | **Fixed**, 2026-09-10 | Found on a real device: `Suspense fallback={null}` showed an empty window for however long the lazy chunk took to fetch and parse, so content popped in a beat after the window opened — a visible layout shift, and it made the tap sound (which already fires on the click itself) read as tied to the content arriving rather than the tap. The data behind each panel was always instant and on-device; only the code to render it was lazy. `index.tsx` now prefetches those three chunks (not the Hunter License's, which F9 still defers on purpose) shortly after Status mounts, confirmed via network timing that all three fetch together well before any summon click, not during one |
