...
## Where the build stands (M3 commits 1-3 landed 2026-09-04)

The engine, store and rest-timer corrections from `docs/m3-plan.md` (G1-G6) are in: the rep-range
override, the routine-from-session fix, the bodyweight default, the warmup correction, the
`order` fix, and the pure rest-timer module. None of them needed the gate screen to exist, so they
landed first. Commits 4-7 (the gate screen itself, set entry, set correction, and the rest timer
wired up with the wake lock and chime) are next.
...
- [~] G6 The rest timer cannot use `setInterval`: hidden tabs throttle to ~once a minute and squat
      ...
      The pure arithmetic (`src/app/rest-timer.ts`) is done; wiring it into a screen with the wake
      lock and chime is M3 commit 7
