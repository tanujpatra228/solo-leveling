...
| Every `detectAdvisories` structural gap closed in the seed week | **Done**, landed 2026-09-12 | `no-hip-hinge`, `hamstrings-below-mev`, `no-cuff-prehab`, `no-grip-work`, `no-unilateral-lower` and `push-leg-day-imbalance` all used to fire against the real seed routines; none do now. Added six prescribed exercises: Romanian Deadlift and Barbell Hip Thrust (Friday, the missing hinge and a dedicated glute driver), Cable External Rotation (Monday and Thursday, after each pressing block), Farmer's Carry (Tuesday, direct grip), Dumbbell Bulgarian Split Squat (Wednesday — deliberately the *second* squat/hinge/lunge day rather than a third exercise on Friday, since that is what actually closes the push-day-to-leg-day frequency gap), and Pallof Press (Wednesday, swapped in for Machine Abs Crunch — the week's core work was flexion-only). Also promoted Nordic Curl from the fallback library to prescribed (Friday): real injury-prevention evidence (Petersen et al. 2011) for a movement that was seeded but never actually programmed. Four new fallbacks (Dumbbell External Rotation, Dead Hang, Side Plank, Glute Bridge) exist only so `substitutesFor` has an equipment-free answer for the four muscles — rotator cuff, grip, obliques, glutes — that went from zero to exactly one prescribed exercise; `cable-external-rotation` and `farmers-carry` are still genuine bodyweight-only gaps (added to `substitution.test.ts`'s exception list, same as pull-ups) since cuff and carry work both need an implement. `advisories.test.ts` and `substitution.test.ts` updated to match — several of its assertions existed specifically to pin down the gaps this closes |
| Exercise help: how-to steps, common mistakes, and a muscle-map visual | **Done**, landed 2026-09-13 | A "?" next to every exercise on the live Gate screen (`gate.tsx`'s `ActiveBlockItem`, via the existing `HelpButton`) opens `ExerciseHelpModal` — a `SystemOverlay` showing the exercise's existing one-line `cue`, a new setup/steps/common-mistakes guide, and `MuscleMap`, a body diagram lit up by primary (bright) and secondary (dim) muscles. Guide copy lives in `content/exerciseGuides.ts`, keyed by exercise id rather than merged into the `Exercise` schema, covering all 41 *prescribed* exercises — fallback-library exercises are intentionally not covered yet (`ExerciseHelpModal` shows an honest "not written yet" rather than a placeholder for those); `exerciseGuides.test.ts` asserts every prescribed exercise has a guide and every guide resolves to a real id |
| The muscle-map diagram, replaced with a real anatomical atlas | **Done**, landed 2026-09-14 | The first version (above) hand-drew the body as flat rectangles — functional, but visibly blocky against a reference the hunter actually wanted matched. Replaced same-day with `components/anatomy/BodyMap.tsx` + two traced-anatomy SVGs (anterior/posterior, ~90 individually id'd regions), user-supplied. `muscleMapRegions.ts` maps this app's 20 `Muscle` values onto that atlas's region ids — 19 of them now light up a real traced shape (`rotator_cuff` included, via `rotator-cuff-infraspinatus-teres-region` — a region the rectangle version couldn't offer at all); only `cardio` still renders as a labelled badge, since it isn't a muscle. `MuscleMap.test.tsx` asserts every id this app references actually exists in the SVG it targets, in each direction, so a typo in the mapping fails a test rather than silently highlighting nothing. Found and fixed while integration-testing: `BodyMap`'s scoped-CSS-injection approach used a fixed `body-map-${view}` class rather than a per-instance one, so two mounted instances of the same view would cross-colour each other's muscles — invisible with the app's actual one-modal-at-a-time usage, but a real latent bug; fixed with `useId()`. The atlas is detailed enough that inlining both SVGs raw cost ~98 KB gzipped on the initial route (measured), so `ExerciseHelpModal` (and everything under it, including `MuscleMap`) is now lazy-loaded from the Gate screen the same way `HunterLicenseCard` already is (index.tsx) — initial-route cost back to ~217.6 KB gzipped, with the anatomy atlas paid for in its own 106.79 KB gzipped chunk only the first time a hunter actually opens a "?" |
| Same diagram: themed properly, and the anterior view's missing outlines | **Done**, landed 2026-09-14 | Two problems the first atlas pass left standing, found on review. First: every untouched region rendered in the source SVGs' own light grey/white — a clinical-chart palette, not this app's. Second: the anterior (front) view had no visible muscle boundaries at all, while the posterior (back) view did. Traced the second one into the actual source files rather than guessing: posterior gets its crisp lines from dedicated stroke-only `*-outline` paths (`anatomy-full-body-outline`, `trapezius-outline`, `latissimus-dorsi-outline`, `posterior-deltoid-outline`, `rotator-cuff-region-outline`) layered on top; anterior has no equivalent — its one `anatomy-outer-outline` id, isolated and rendered alone to check, turned out to be an unrelated foot-area shape, not a body outline. Rather than wait on new source art, both problems are fixed the same way, in code: `BodyMap` (`components/anatomy/BodyMap.tsx`) now takes optional `baseFill`/`baseStroke` props, emitted as one unconditional `.scope path { fill; stroke }` rule — lower CSS specificity than the existing per-muscle `#id path` override rules, so highlighted muscles still win on `fill`, and every region (highlighted or not) picks up the same stroke, on both views alike. `stroke-width` uses `vector-effect: non-scaling-stroke` since the source viewBox is ~600x1100 and a width specified in that space would round to sub-pixel at display size. `MuscleMap.tsx` supplies the actual theme values (`var(--color-panel-edge)` fill, a dimmed `var(--color-ink-faint)` stroke) — `BodyMap` itself stays palette-agnostic. Also addressed: the lazy-loaded `ExerciseHelpModal`'s `Suspense fallback={null}` meant tapping "?" did nothing visible until the chunk resolved. `ExerciseHelpModalSkeleton` (`gate.tsx`) now renders the real, non-lazy `SystemOverlay` chrome immediately — same title, same close control — with pulsing placeholder panels shaped like the real body, same construction as `HunterLicenseCardSkeleton` |
| Farmer's Carry doesn't train what actually produces grip strength | **Fixed**, landed 2026-09-14 | User audit: crushing grip force comes from the forearm flexors crossing the wrist (plus the thenar group), and a carry loads them isometrically at one fixed, already-shortened hand position — no stretch, and it never touches the extensors that stabilise the wrist so the flexors can transmit force at all. Checked the other six exercises added the same week against the same stretch-mediated-hypertrophy standard while at it: Romanian Deadlift, Nordic Curl and the Bulgarian Split Squat are all genuinely stretch-biased already (peak tension at long muscle length, the textbook cases); Cable External Rotation and Pallof Press aren't hypertrophy exercises by design (joint stability, anti-rotation) so the standard doesn't apply; Barbell Hip Thrust is honestly *shortened*-biased (peak glute tension at lockout) and stays anyway — it complements the RDL's stretch bias rather than contradicting it, and has the strongest EMG evidence of any common glute exercise. Fix: added **Wrist Curl** and **Reverse Wrist Curl** (Tuesday, after the carry) — both `primary: forearms`, both loaded from full wrist extension/flexion respectively, the actual stretched position for each direction. This also closed a gap that predates this fix and wasn't caught until now: `forearms` had zero primary-muscle exercises anywhere in the 41, only ever secondary. Farmer's Carry stays (real value for holding endurance and carryover), now genuinely complementary rather than the only thing touching grip. Deliberately not added: a thumb/thenar-specific movement (plate pinches) — `EquipmentSchema` has no "plate" tag, and seed.ts already documents leaving a plate-based exercise out for exactly that reason rather than mistagging it; extended that same precedent here instead of reversing it. `cable-wrist-curl` added as the equipment-free-of-dumbbell fallback for both (`substitution.test.ts`'s exception list, same treatment as cable-external-rotation/farmers-carry, since no bodyweight movement meaningfully loads wrist flexion/extension) |
| Grip work, refined again: radial-biased extension, and the thumb the carry/curls still missed | **Done**, landed 2026-09-14 | Second pass, prompted by a YouTube Short the hunter checked against the codebase's own standard rather than taken on faith. Two claims, two different evidence tiers, both honestly labelled as such: **(1)** Reverse Wrist Curl's cue now specifies extension biased toward the thumb side (radial deviation), not pure sagittal extension — a failing grip collapses toward flexion *and* ulnar deviation (a heavy suitcase pulling the wrist down and pinky-side), so training the exact opposite vector trains ECRL/ECRB specifically rather than spreading the work evenly across all three wrist extensors including ECU, the one least relevant to that failure direction. Sound biomechanical reasoning from grip-sport coaching practice, not a cited RCT — a different, still-legitimate evidence category from the stretch-mediated-hypertrophy meta-analyses behind the RDL/Nordic Curl calls, and said so rather than dressing it up as the same tier. **(2)** Added **Dumbbell Hub Pinch** (Tuesday, end of the block) for the thenar/thumb group (adductor pollicis, flexor pollicis brevis, opponens pollicis) — pinch grip is a genuinely distinct grip quality from the crush strength Farmer's Carry trains and the crossed-wrist muscles the curls train; neither touches it. This is a plate pinch in substance, done with a hex dumbbell instead of a plate for the same equipment-tagging reason `wrist-curl`'s entry already documents — confirmed by asking rather than assuming: the hunter has no kettlebell access, which the video's own version of this exercise needs. Added to `substitution.test.ts`'s bodyweight-only exception list — pinch strength genuinely needs a flat-sided load, same reasoning as the carry |
| Bodyweight gates — a second routine set, chosen by equipment access | **Done**, landed 2026-09-19 | A bodyweight-only hunter used to get the barbell six regardless — routines seeded unconditionally, before a profile (and equipment access) existed. Five phases, detailed in `implementation-plan.md` §4: (0) fixed a live equipment-normalization bug found on the way in — `bodyweight` was a plain selectable, so a full gym without it ticked reached only 41 of 64 exercises; `domain/equipment.ts`'s `effectiveEquipment()`/`isBodyweightProgramme()`/`applyEquipmentSelection()` now own this everywhere. (1) 17 new exercises, 11 fallback→prescribed promotions, across ten progression ladders. (2) `SEED_ROUTINES_BODYWEIGHT`, six `bw-` gates, every exercise performable with `['bodyweight', 'pullup_bar']`, volume-checked against `LANDMARKS` (every reachable muscle clears MEV). (3) routine seeding moved to `reconcileRoutines()` — wholesale replace, only when every existing row is provably untouched and no session is open, remapping cleared-gate history so switching equipment never triggers a Dungeon Break. (4) two new equipment-keyed advisories, `no-pullup-bar` and `no-external-load`. (5) verified end to end in a real browser (Playwright, zero console errors): Awakening → six correct gates → gate started, logged and cleared with a level-up → Analysis panel showing exactly the advisories each equipment scenario predicts |
| The CST split replaced with Push/Pull/Legs | **Done**, landed 2026-09-26 | Found on a real hunter's Analysis screen: the CST/back-biceps/legs split simultaneously fired `biceps-consecutive-days`, `many-variants-biceps:3` and `front-delt-overload` — three warnings the split's own shape guaranteed and no amount of set-shuffling inside it could fix. Designed a PPL x2 split (`SEED_ROUTINES`, `ppl-`-prefixed ids, same `dayOfWeek` values) from the existing prescribed library only — no new exercises — and verified it against the real `detectAdvisories`/`weeklyVolumeReport` before shipping: zero advisories fire, every muscle the split trains clears its MEV floor (traps stays below floor, same pre-existing gap the CST split had; grip runs a little hot but stays under its recoverable ceiling). The CST split's exact content is preserved as `LEGACY_SEED_ROUTINES_CST`, referenced only by `reconcileRoutines`'s untouched-row proof, so an already-onboarded barbell hunter's stored CST rows are still recognized as safe to replace — that migration path is what actually gets the new split onto an existing device; a hunter who never re-opens the app or whose rows were hand-edited (impossible today — no edit path exists) would not receive it. `reconcileRoutines`'s own doc comment now states the one real limitation this surfaced: a content-only correction to a routine that *keeps its id* can never reach an already-seeded hunter through this function, since the untouched-check compares against the same live seed shape the early return already did — only an id change (as this migration used) does. Verified end to end in a real browser: a fresh barbell Awakening shows all six PPL gates, no CST gate anywhere |
...
| Bundle | 213.76 KB JS + 6.60 KB CSS + 2.20 KB `workbox-window` ≈ 222.6 KB gzipped initial route, under half the ~480 KB Slow-4G budget. The Hunter License card, the QR scanner, and `ExerciseHelpModal` (the anatomy atlas SVGs live here — 111.84 KB gzipped, paid only the first time a hunter opens a "?") ship as their own lazy chunks — Tower, Shadow Army and Shop were folded back into the main bundle 2026-09-10 (each under 1 KB, not worth Suspense's timing cost) |
