...
| Exercise substitution, round 2: equipment-desert re-audit | **Done**, landed 2026-09-11 | Re-ran the commit 5d33216 coverage method against every muscle and pattern, not just the equipment-desert test. Found: no loaded hinge movement anywhere in the library (no Romanian deadlift), no exercise naming glutes as a primary mover at all, `horizontal_pull`/`upper_back` had no cable or machine answer, chest and front_delts pressing had no free-weight fallback once barbell and cable were both occupied, biceps had no dumbbell curl, the `carry` pattern was unused, and `bands` — offered as equipment since the Awakening Test's picker — had never been asked for by a single exercise. Added 15 `fallback`-role exercises to `src/db/seed.ts` (Dumbbell Bench Press, Dumbbell Shoulder Press, Bench Dips, Dumbbell Bicep Curl, Seated Cable Row, Face Pull, Dumbbell Reverse Fly, Dumbbell Goblet Squat, Barbell and Dumbbell Romanian Deadlift, Barbell Hip Thrust, Seated Calf Raise Machine, Farmer's Carry, Band External Rotation, Band Pull-Apart) — never prescribed, never a routine default, same rule as the original fallback library. `seed.test.ts`'s `GROUPS_FROM_AUDIT` extended with `glutes`, `grip`, `forearms`, `rotator_cuff` so the existing per-fallback test enforces the new coverage the same way it enforces the old |
...
| Bundle | 208.91 KB JS + 6.45 KB CSS + 2.20 KB `workbox-window` ≈ 217.6 KB gzipped initial route, under half the ~480 KB Slow-4G budget. Only the Hunter License card and the QR scanner still ship as their own lazy chunks — Tower, Shadow Army and Shop were folded back into the main bundle 2026-09-10 (each under 1 KB, not worth Suspense's timing cost) |
